...
Отслеживает скорость каждого подключённого IP, и если кто-то начинает выжимать канал — автоматически режет ему скорость. Если клиент продолжает упираться в лимит — лимит ужесточается. Когда трафик утихает — шейп снимается. Сам Telegram при этом не трогается.

## Зачем это

MTProto-прокси любят использовать как «бесплатный VPN» — гонять через него торренты, видео, всё что угодно. Один такой клиент может выжать всю полосу сервера и положить опыт остальным. `telemt-shaper` ловит таких и аккуратно режет им скорость, не мешая обычным пользователям Telegram.
